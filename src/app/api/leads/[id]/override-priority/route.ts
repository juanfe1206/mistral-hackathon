...
import * as overrideService from "@/server/services/override-service";
import type { LeadPriority } from "@prisma/client";

const overrideBodySchema = z.object({
  priority: z.enum(["vip", "high", "low"]),
  reason: z.string().max(500).optional(),
});

/**
 * POST /api/leads/[id]/override-priority
 * Apply manual priority override with audit trail.
 */
export async function POST(
  ...
  try {
    ...
    await overrideService.overridePriority({
      leadId: id,
      tenantId,
      newPriority: priority as LeadPriority,
      reason: reason ?? null,
    ...
    if (!lead) {
      return NextResponse.json(
        createErrorResponse("FETCH_FAILED", "Lead not found after override", [], requestId),
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      createErrorResponse("OVERRIDE_FAILED", "Priority override failed", [message], requestId),
      { status: 500 }
    );
  }
}
